...
struct coap_resource_t *epoch_resource = NULL;
...
/* Sensor data storage before they are insterted into db */
typedef struct {
	uint32_t utc;
...
/* Provioned for gateway statistics */
typedef struct {
...
/* Configuration pertaining procedures */
...
/* Gateway authentication procedures */
...
#define INDEX "This is a LiteIoT CoAP Gateway based on libcoap CoAP protocol implementation\n" \
              "Vladislav Rykov <vrykov@uji.es>\n\n"
...
static void hnd_get_epoch(coap_context_t *ctx UNUSED_PARAM,
              struct coap_resource_t *resource,
              coap_session_t *session,
              coap_pdu_t *request,
              coap_binary_t *token,
              ...
              coap_pdu_t *response) 
{
	...
	char *pak; // pointer to app_key
	uint8_t buf[64];
	uint8_t buf_len = 0;

	/* first ocurrence must be given by app_key=******** */
	pak = memchr(query->s, '=', strlen((char *)query->s)-1);
	memcpy(gwp_conf.app_key, &pak[1], GATEWAY_PROTOCOL_APP_KEY_SIZE);
	gwp_conf.app_key[GATEWAY_PROTOCOL_APP_KEY_SIZE] = '\0';
	
	pak = &pak[GATEWAY_PROTOCOL_APP_KEY_SIZE+1];
	pak = memchr(pak, '=', strlen(pak)-1);
	gwp_conf.dev_id = atoi(&pak[1]);
...
static void hnd_post_data(coap_context_t *ctx UNUSED_PARAM,
              struct coap_resource_t *resource,
              coap_session_t *session,
              coap_pdu_t *request,
              coap_binary_t *token,
              ...
              coap_pdu_t *response) 
{
	...
	coap_get_data(request, &packet_length, &packet);

	if (packet_length == 0) {
		// bad request 400
		response->code = COAP_RESPONSE_CODE(400);
		fprintf(stderr, "error : no incoming data\n");
	} else {
		...
		// here comes packet processing
		if (gateway_protocol_packet_decode(
			&gwp_conf,
			NULL,
			&payload_length, payload,
			packet_length, (uint8_t *)packet))
		{
			sensor_data_t sensor_data;
			time_t t;
			...
			snprintf(db_query, sizeof(db_query), 
				...
				(char *)gwp_conf.app_key, gwp_conf.dev_id, t, sensor_data.timedate
			);
			...
			gw_stat_linked_list_add((char *)gwp_conf.app_key, gwp_conf.dev_id);
			...
			if (PQresultStatus(res) == PGRES_COMMAND_OK) {
				...
				if (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res)) {
					/* there are updated for the device */
					// changed 204
...
static void hnd_get_data(coap_context_t *ctx UNUSED_PARAM,
              struct coap_resource_t *resource,
              coap_session_t *session,
              coap_pdu_t *request,
              coap_binary_t *token,
...
static int get_uri_proxy_scheme_info(coap_pdu_t *request,
                          coap_opt_t *opt,
                          coap_uri_t *uri,
                          ...
                          coap_string_t **uri_query)
{
  	...
  	if (opt_len == 9 && strncasecmp(opt_val, "coaps+tcp", 9) == 0) {
    		uri->scheme = COAP_URI_SCHEME_COAPS_TCP;
    		uri->port = COAPS_DEFAULT_PORT;
  	} else if (opt_len == 8 && strncasecmp(opt_val, "coap+tcp", 8) == 0) {
    		uri->scheme = COAP_URI_SCHEME_COAP_TCP;
    		uri->port = COAP_DEFAULT_PORT;
  	} else if (opt_len == 5 && strncasecmp(opt_val, "coaps", 5) == 0) {
    		uri->scheme = COAP_URI_SCHEME_COAPS;
    		uri->port = COAPS_DEFAULT_PORT;
  	} else if (opt_len == 4 && strncasecmp(opt_val, "coap", 4) == 0) {
  	...
  	if (opt) {
    		uri->port = coap_decode_var_bytes (coap_opt_value (opt),
                                 coap_opt_length (opt));
  	}
...
static void init_resources(coap_context_t *ctx) {
  ...
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"LiteIoT CoAP Gateway\""), 0);
  coap_add_resource(ctx, r);

  // get epoch
  r = coap_resource_init(coap_make_str_const("epoch"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_epoch);
  
  coap_add_attr(r, coap_make_str_const("ct"), coap_make_str_const("42"), 0);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Seconds since January 1 1970\""), 0);
  coap_add_attr(r, coap_make_str_const("rt"), coap_make_str_const("\"seconds\""), 0);
  ...
  // post & get data (data_send & pend_req)
  r = coap_resource_init(coap_make_str_const("data"), resource_flags);
  coap_register_handler(r, COAP_REQUEST_POST, hnd_post_data);
  coap_register_handler(r, COAP_REQUEST_GET, hnd_get_data);
  
  coap_add_attr(r, coap_make_str_const("ct"), coap_make_str_const("42"), 0);
  coap_add_attr(r, coap_make_str_const("title"), coap_make_str_const("\"Interface for sending senosor's data and requesting device's configuration.\""), 0);
...
int main (int argc, char **argv) {
	...
	snprintf(port_str, NI_MAXSERV, "%u", gw_conf->static_conf.gw_port);
  	ctx = get_context(addr_str, port_str);
  	if (!ctx)
    		return -1;
  	...
  	while ( !quit ) {
    		...
    		if (epoch_resource) {
      			...
      			if (t_last != t_now) {
        			/* Happens once per second */
        			t_last = t_now;
	  			coap_resource_notify_observers(epoch_resource, NULL);
      			}
      			/* need to wait until next second starts if wait_ms is too large */
      			next_sec_ms = 1000 - (now % COAP_TICKS_PER_SECOND) *
                           			1000 / COAP_TICKS_PER_SECOND;
