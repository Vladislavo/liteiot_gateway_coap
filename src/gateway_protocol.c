...
static gateway_protocol_checkup_callback_t checkup_callback = NULL;
    ...
    uint8_t *packet)
{
    ...
    if (checkup_callback && checkup_callback(gwp_conf)) {
	if (gwp_conf->secure) {
    ...
    if (packet_type) *packet_type = (gateway_protocol_packet_type_t) packet[p_len];
    p_len++;
